...
router.post("/update", [checkDataExisting(["ID_general_plan", "code", "description"])], async (req, res) => {
    ...
    try {
        await connection.execute("UPDATE plany_ogolne SET kod = ?, opis = ? WHERE ID = ?", [code, description, ID_general_plan]);
...
router.post("/delete", [checkDataExisting(["ID_general_plan"])], async (req, res) => {
    ...
    try {
        await connection.execute("DELETE FROM plany_ogolne where ID = ?", [ID_general_plan]);
...
router.post("/insert", [checkDataExisting(["code", "description"])], async (req, res) => {
    ...
    try {
        await connection.execute("INSERT INTO plany_ogolne() values(NULL, ?, ?)", [code, description]);
...
router.post("/insert_many", [checkDataExisting(["data"])], async (req, res) => {
    const {data} = req.body;
    try {
        const [result] = await connection.execute("SELECT kod FROM plany_ogolne");
        Object.keys(data).forEach(async (key) => {
            if(!result.some((value) => key == value.kod)) {
                await connection.execute("INSERT INTO plany_ogolne() values(NULL, ?, ?)", [key, data[key]]);
            }
        });
