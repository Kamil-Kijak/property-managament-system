...
router.get("/get", [checkDataExisting(["ID_land"])], async (req, res) => {
    ...
    try {
        const [result] = await connection.execute("SELECT pd.powierzchnia, k.klasa, k.przelicznik FROM powierzchnie_dzialek pd INNER JOIN klasy_gruntu k ON pd.ID_klasy=k.ID", [ID_land]);
...
router.post("/insert", [checkDataExisting(["ID_land", "ID_ground_class", "area"])], async (req, res) => {
    const {ID_land, ID_ground_class, area} = req.body;
    try {
        await connection.execute("INSERT INTO powierzchnie_dzialek VALUES(NULL, ?, ?, ?)", [ID_land, ID_ground_class, area]);
