...
export default function SimpleTextArea({title = "title", placeholder="placeholder...", value = "", onChange=() =>{}, error = null}) {
    return (
        <section className="flex flex-col items-start mb-5 relative">
            ...
            <textarea onChange={onChange}  placeholder={placeholder} className={`border-3 h-[6rem] w-full ${error ? "border-red-700" : "border-black"} p-2 rounded-md bg-white resize-none`} value={value || ""}></textarea>
